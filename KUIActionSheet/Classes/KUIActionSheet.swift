...
extension KUIActionSheetNibLoadableView where Self: KUIActionSheet {
    ...
    public static func viewWithNibName(nibFileName: String,  parentViewController viewController: UIViewController, theme: KUIActionSheetProtocol = KUIActionSheetDefault()) -> KUIActionSheetNibLoadableView? {
        let views = Bundle(for: self).loadNibNamed(nibFileName, owner: nil, options: nil)
        for view in views! {
...
open class KUIActionSheet: UIView {
    ...
    @IBOutlet open weak var cancelButtonBottom: NSLayoutConstraint!
    ...
    open class func view(parentViewController viewController: UIViewController, theme: KUIActionSheetProtocol = KUIActionSheetDefault()) -> KUIActionSheet? {
        let views = Bundle(for: self).loadNibNamed("KUIActionSheet", owner: nil, options: nil)
        for view in views! {
    ...
    open func show(_ viewController: UIViewController? = nil, completion: ((Bool) -> Void)? = nil) {
        guard !showing else {
            completion?(false)
            return
        }
    ...
    open func dismiss(_ completion: ((Bool) -> Void)? = nil) {
        guard showing else {
            completion?(false)
            return
        }
    ...
    open func add(item: KUIActionSheetItem) {
        ...
        button.setTitleColor(item.destructive ? itemTheme.destructiveTitleColor : itemTheme.titleColor, for: UIControlState())
    ...
    internal func singleTap(_ gesture: UITapGestureRecognizer) {
        ...
        guard gesture.location(in: self).y < containerView.frame.minY && tapToDismiss else { return }
        dismiss()
    }
...
private class KUIActionSheetItemButton: UIButton, KUIActionSheetItemViewProtocol {
    ...
    fileprivate var item: KUIActionSheetItem! {
        willSet {
